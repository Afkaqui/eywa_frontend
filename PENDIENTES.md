...
| **3** | ✅ **HECHA (2026-08-11, `9e9f4a3`).** `OrgSwitcher` (selector de organización activa), campos **RUC** y **nombre comercial** en el formulario, y repositorio + proxies completos. El selector **solo aparece si hay más de una organización o si puede agregar otra**: con una sola, la vista queda idéntica. La organización activa vive en `localStorage` (es preferencia de vista, no dato de negocio) y cae a la predeterminada si la guardada ya no existe. Se corrigió que el formulario tragaba el error del backend y mostraba un genérico: ahora muestra el mensaje real, que con la validación de RUC es justo lo que hay que leer | Medio |
| **3b** | ✅ **HECHA (2026-08-22).** Diagnóstico, Dataroom, panel principal y Mi Organización operan sobre la empresa elegida. Se corrigieron **cuatro fallos que hacían el selector decorativo** y que solo aparecieron al probarlo en el navegador: (a) *Agregar organización* no hacía nada — `OrgSwitcher` llama `onAdd?.()` y `OrganizationProfile` no le pasaba ninguno, así que **no había forma de dar de alta una segunda empresa desde la interfaz**; (b) Mi Organización cargaba con `get()` y guardaba con `PUT`, ambos sobre la predeterminada — cambiar de empresa no tenía efecto y editar la segunda habría pisado la primera (ahora `getFull(activa)` + `PATCH /:id`, `POST` si es nueva); (c) el Dataroom resolvía la organización en 10 endpoints con `getOrganizationOf` (la más antigua): **los documentos de la segunda empresa se habrían subido a la primera**; (d) `/stats/me` mezclaba — el Trust Score salía del último diagnóstico de CUALQUIER empresa mientras la completitud del Dataroom era la de la más antigua. Nuevo `GET /organization/:id`; se eliminó `resolveOrg()` sin usos. **Verificado en producción con cuentas desechables:** subir a la segunda empresa deja el archivo en la segunda y la primera vacía, subir a empresa ajena da 404, editar la segunda no toca la primera, y los paneles de las dos empresas devuelven su propio índice (2,0 vs 4,53) | Medio |
