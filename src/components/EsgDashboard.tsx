...
export function EsgDashboard({ embedded = false }: { embedded?: boolean }) {
  ...
  if (loading) {
    return (
      <div className={embedded ? 'flex items-center justify-center py-20' : 'min-h-screen flex items-center justify-center bg-gray-50'}>
  ...
  return (
    <div className={embedded ? '' : 'min-h-screen bg-gray-50'}>
      {/* Header — hidden when embedded (parent provides its own header) */}
      {!embedded && (
      ...
      {/* Tabs — shown inline when embedded */}
      {embedded && (
