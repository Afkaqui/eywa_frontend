...
// Descarga binaria del documento (proxyToBackend hace .json() y corrompería el archivo).
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string; docId: string }> }) {
  ...
  try {
    const res = await fetch(`${API_URL}/api/validator/plans/${id}/documents/${docId}/download`, {
